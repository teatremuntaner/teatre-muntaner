...
(function () {
  ...
  function itemsDe(slot){
    ...
    if (slot.lista && slot.lista.length){
      for (var i=0;i<slot.lista.length;i++){ var it=slot.lista[i]; if(it && (it.archivo||it.url)) its.push({archivo:it.archivo, url:it.url, segundos:(+it.segundos||0), girar:(it.girar!==false), efecto:it.efecto, overlay:it.overlay, relleno:it.relleno, show:it.show}); }
  ...
  function cargarFunciones(){
    ...
    fetch(FUNC_URL + (FUNC_URL.indexOf('?')<0?'?':'&') + 't=' + Date.now())
      ...
      .then(function(j){ var m={}; ((j&&j.shows)||[]).forEach(function(s){ if(s&&s.id) m[s.id]=s; }); funcs=m; })
      .catch(function(){});
  }
