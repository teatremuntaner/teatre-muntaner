...
// Efectos que entiende motor.js (cualquier otro valor se descarta al guardar).
$GLOBALS['EFECTOS_OK']  = ['kbZoomOut','kbDriftR','kbDriftL','kbDriftU','kbPop','kbWow'];
$GLOBALS['OVERLAYS_OK'] = ['shine','glow','sparkle','flash','none'];
function limpia_lista($lista){
  $out = [];
  if (is_array($lista)) foreach ($lista as $it){
    $seg = max(0, min(3600, (int)($it['segundos'] ?? 0)));
    $gir = array_key_exists('girar', $it) ? !empty($it['girar']) : true;
    // hueco AUTOMÁTICO: lo resuelve la propia tele (motor.js) con funciones.json +
    // config.robot; aquí solo se conserva. criterio: proximo | prioridad | azar.
    if (!empty($it['auto'])) {
      $e = ['auto'=>true, 'segundos'=>$seg, 'girar'=>$gir];
      if (!empty($it['criterio']) && in_array($it['criterio'], ['proximo','prioridad','azar'], true)) $e['criterio'] = $it['criterio'];
      $out[] = $e; continue;
    }
    if (empty($it['archivo'])) continue;
    $e = ['archivo'=>basename($it['archivo']), 'segundos'=>$seg, 'girar'=>$gir];
    if (!empty($it['show']))    $e['show']    = preg_replace('/[^a-z0-9\-]/','', strtolower((string)$it['show']));
    if (!empty($it['relleno'])) $e['relleno'] = basename($it['relleno']);
    if (!empty($it['efecto'])  && in_array($it['efecto'],  $GLOBALS['EFECTOS_OK'],  true)) $e['efecto']  = $it['efecto'];
    if (!empty($it['overlay']) && in_array($it['overlay'], $GLOBALS['OVERLAYS_OK'], true)) $e['overlay'] = $it['overlay'];
    $out[] = $e;
  }
  return $out;
}
function html_pantalla($slug, $nombre, $MARCA){
  ...
  return "<!DOCTYPE html>\n$MARCA\n<html lang=\"es\"><head>"
    ...
    ."<script>window.PANTALLA='$s';</script><script src=\"motor.js?v=21\"></script></body></html>\n";
}
...
if ($action !== '') {
  ...
  if ($action === 'estado') {
    ...
    echo json_encode([
      'ok'        => true,
      'rev'       => file_exists($CONFIG) ? (string)filemtime($CONFIG) : '0',
      'config'    => $cfg,
  ...
  if ($action === 'guardar' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Candado contra pestañas desactualizadas: si el config cambió desde que este
    // panel se abrió (otra persona, otra pestaña), NO se machaca lo último.
    $revCli = (string)($body['rev'] ?? '');
    $revSrv = file_exists($CONFIG) ? (string)filemtime($CONFIG) : '0';
    if ($revCli === '' || $revCli !== $revSrv) {
      http_response_code(409);
      echo json_encode(['ok'=>false,'msg'=>'Este panel se quedó desactualizado (alguien guardó después de abrirlo tú). Recarga la página (F5) y vuelve a hacer tu cambio.']); exit;
    }
    $limpio = ['pantallas' => []];
    ...
    if (is_array($prev)) {
      ...
      // 'robot' = reglas de los huecos automáticos (qué cartel usa cada espectáculo,
      // exclusiones, efectos). Las lee motor.js para resolver los huecos {auto}.
      if (!empty($prev['robot']) && is_array($prev['robot'])) $limpio['robot'] = $prev['robot'];
    }
    ...
    clearstatcache();
    echo json_encode(['ok'=>true, 'rev'=>(string)filemtime($CONFIG)]); exit;
  }
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  /* Editor de la lista: una rejilla de columnas fijas para TODAS las filas
     (miniatura · nombre · duración · girar · rótulo/modo · controles) → los
     controles caen siempre en la misma columna, alineados como una tabla. */
  .pllist{--pl-cols:38px minmax(0,1fr) 60px 44px 190px 118px;display:flex;flex-direction:column;gap:5px}
  .plhead{display:grid;grid-template-columns:var(--pl-cols);gap:11px;align-items:center;padding:2px 12px;font-size:.66rem;font-weight:600;text-transform:uppercase;letter-spacing:.05em;color:var(--dim)}
  .plhead .h-c{text-align:center}
  .plitem{display:grid;grid-template-columns:var(--pl-cols);gap:11px;align-items:center;background:var(--panel);border:1px solid var(--line);border-radius:10px;padding:8px 12px;transition:border-color 150ms ease,background 150ms ease}
  .plitem:hover{border-color:#cfc7b8}
  .plitem.autoitem{background:color-mix(in srgb,var(--acc) 5%,var(--panel));border-color:color-mix(in srgb,var(--acc) 28%,var(--line))}
  .pl-thumb{width:38px;height:52px;display:flex;align-items:center;justify-content:center}
  .pl-thumb img{width:38px;height:52px;object-fit:cover;border-radius:5px;background:#000;border:1px solid var(--line);cursor:zoom-in}
  .pl-thumb .ico{font-size:1.4rem;cursor:zoom-in}
  .pl-thumb .ico.badge-auto{cursor:default}
  .pl-name{min-width:0}
  .pl-name .arch{display:block;font-weight:600;font-size:.84rem;line-height:1.25;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  .pl-name .cap{display:block;font-size:.72rem;color:var(--dim);line-height:1.3;margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  .autoitem .pl-name .cap{color:var(--acc2)}
  .pl-seg{display:flex;align-items:center;justify-content:flex-end;gap:4px;color:var(--dim);font-size:.74rem}
  .pl-seg input{width:40px;padding:4px 5px;text-align:right;font-size:.78rem}
  .pl-gir{display:flex;justify-content:center}
  .pl-gir input{width:17px;height:17px;cursor:pointer;accent-color:var(--acc)}
  .pl-sel select{width:100%;font-size:.76rem;padding:5px 7px}
  .plitem .ctrl{display:flex;justify-content:flex-end;gap:1px}
  .plitem .ctrl .btn{border:1px solid transparent;background:transparent;color:var(--dim);padding:5px 6px;font-size:.82rem;border-radius:7px;transition:color 120ms ease,background 120ms ease,transform 120ms ease}
  .plitem .ctrl .btn:hover{color:var(--txt);background:var(--panel2)}
  .plitem .ctrl .btn:active{transform:scale(.92)}
  .plitem .ctrl .btn.danger:hover{color:#fff;background:var(--bad)}
  @media(max-width:640px){
    .plhead{display:none}
    .plitem{grid-template-columns:34px minmax(0,1fr) auto;grid-auto-rows:auto;row-gap:8px}
    .pl-seg{grid-column:2}.pl-gir{grid-column:2;justify-content:flex-start}.pl-sel{grid-column:1/-1}
    .plitem .ctrl{grid-column:3;grid-row:1}
  }
  .pl-add{display:flex;align-items:center;gap:8px;flex-wrap:wrap;border:1.5px dashed var(--line);border-radius:10px;padding:11px 12px;color:var(--dim);margin-top:8px;font-size:.82rem}
  .pl-add.drag{border-color:var(--acc);background:color-mix(in srgb,var(--acc) 8%,transparent)}
  .pl-add select{max-width:170px}
  .pl-add .grow{flex:1}
...
<script>
// ════════════════════ Estado ════════════════════
var CLAVE='', BASE='', URL_TELE='', limite=0, REV='';
var archivos=[];
...
function cargarShows(){
  ...
  fetch(u+(u.indexOf('?')<0?'?':'&')+'t='+Date.now()).then(function(r){ return r.json(); })
    .then(function(j){ shows=((j&&j.shows)||[]).map(function(s){ return {id:s.id, title:s.title, future:s.future||[], priority:(+s.priority||0)}; }); render(); })
    .catch(function(){ shows=[]; });
}
// ── Huecos AUTOMÁTICOS: la misma regla que aplica la tele (motor.js). config.robot
//    dice qué cartel usa cada espectáculo (map), cuáles se excluyen y con qué efectos
//    se pintan. Criterios: proximo (función más cercana) · prioridad (jerarquía de
//    cartelera) · azar (rota cada día). El panel solo lo simula para enseñarte qué
//    saldría; quien resuelve de verdad es la propia pantalla.
var CRITERIOS={proximo:'el más próximo', prioridad:'por prioridad', azar:'sorpresa (rota a diario)'};
function robotCfg(){ return config.robot||{}; }
function candidatosAuto(usados){
  var r=robotCfg(), ex=r.excluir||[], hoy=ymd(new Date()), out=[];
  shows.forEach(function(s){
    if(ex.indexOf(s.id)>=0 || usados[s.id]) return;
    var fut=(s.future||[]).filter(function(f){ return f&&f.date&&f.date>=hoy; });
    if(!fut.length) return;
    if(!(r.map||{})[s.id]) return;
    out.push({id:s.id, prox:fut[0].date, prio:(+s.priority||0), titulo:s.title||s.id});
  });
  return out;
}
...
// resuelve TODOS los autos de una lista en orden (cada hueco consume su espectáculo)
function resuelveAutosDe(lista){
  var usados={}, out={};
  (lista||[]).forEach(function(it,i){
    if(!it||!it.auto) return;
    var el=eligeAuto(it.criterio||'proximo', i, usados);
    if(el){ usados[el.id]=1; el.archivo=(robotCfg().map||{})[el.id]; }
    out[i]=el;
  });
  return out;
}
...
function renderContenido(t){
  ...
  if(web){
    ...
    var autosRes=resuelveAutosDe(L);
    var ctrlDe=function(i){
      return '<div class="ctrl"><button class="btn" title="Subir" onclick="slotItemMover(\''+t+'\','+i+',-1)">▲</button>'+
        '<button class="btn" title="Bajar" onclick="slotItemMover(\''+t+'\','+i+',1)">▼</button>'+
        '<button class="btn" title="Duplicar" onclick="slotItemDup(\''+t+'\','+i+')">⧉</button>'+
        '<button class="btn danger" title="Quitar" onclick="slotItemDel(\''+t+'\','+i+')">✕</button></div>';
    };
    var segCell=function(i,it,vid){
      var inp = vid
        ? '<input type="number" min="0" placeholder="—" value="'+(it.segundos>0?it.segundos:'')+'" title="segundos (vacío = vídeo entero)" onchange="slotItemSeg(\''+t+'\','+i+',this.value)">'
        : '<input type="number" min="1" value="'+(it.segundos>0?it.segundos:10)+'" onchange="slotItemSeg(\''+t+'\','+i+',this.value)">';
      return '<div class="pl-seg">'+inp+' s</div>';
    };
    var girCell=function(i,it){
      if(!rota) return '<div></div>';
      return '<div class="pl-gir"><input type="checkbox" '+(it.girar!==false?'checked':'')+' title="Girar para la pantalla (márcalo si el cartel está derecho)" onchange="slotItemGirar(\''+t+'\','+i+',this.checked)"></div>';
    };
    var filas=L.map(function(it,i){
      if(it.auto){
        total+=(it.segundos>0?it.segundos:10);
        var crit=it.criterio||'proximo';
        var critSel='<div class="pl-sel"><select title="Cómo se elige el espectáculo" onchange="slotItemCriterio(\''+t+'\','+i+',this.value)">'+
          Object.keys(CRITERIOS).map(function(k){ return '<option value="'+k+'"'+(k===crit?' selected':'')+'>'+CRITERIOS[k]+'</option>'; }).join('')+'</select></div>';
        var res=autosRes[i];
        var estado=res
          ? 'ahora: <b>'+(res.titulo||res.id)+'</b> · '+res.prox+(res.archivo?'':' · <span style="color:var(--bad)">sin cartel</span>')
          : '<span style="color:var(--bad)">sin espectáculo con cartel y fecha</span>';
        return '<div class="plitem autoitem">'+
          '<div class="pl-thumb"><span class="ico badge-auto" title="Hueco automático">🎭</span></div>'+
          '<div class="pl-name"><span class="arch">Hueco automático</span><span class="cap">'+estado+'</span></div>'+
          segCell(i,it,false)+girCell(i,it)+critSel+ctrlDe(i)+'</div>';
      }
      var v=it.archivo, vid=esVideo(v);
      if(!vid) total+=(it.segundos>0?it.segundos:10); else if(it.segundos>0) total+=it.segundos;
      var pn=v.replace(/'/g,"\\'");
      var thumb = vid?'<span class="ico vid" onclick="abrirLightbox(\''+pn+'\')" title="Ver">🎬</span>':'<img src="'+urlDe(v)+'" onclick="abrirLightbox(\''+pn+'\')">';
      var cap = vid?'<span class="cap" id="dur_'+t+'_'+i+'"></span>':'';
      var showSel='<div class="pl-sel"><select title="Espectáculo (para mostrar su próxima función encima del cartel)" onchange="slotItemShow(\''+t+'\','+i+',this.value)">'+opcionesShow(it.show)+'</select></div>';
      return '<div class="plitem">'+
        '<div class="pl-thumb">'+thumb+'</div>'+
        '<div class="pl-name"><span class="arch" title="'+v.replace(/"/g,'&quot;')+'">'+v+'</span>'+cap+'</div>'+
        segCell(i,it,vid)+girCell(i,it)+showSel+ctrlDe(i)+'</div>';
    }).join('');
    var head='<div class="plhead"><span></span><span></span><span class="h-c">seg.</span>'+(rota?'<span class="h-c">girar</span>':'<span></span>')+'<span>espectáculo / modo</span><span></span></div>';
    var add='<div class="pl-add" ondragover="permitir(event);this.classList.add(\'drag\')" ondragleave="this.classList.remove(\'drag\')" ondrop="soltarEnPlaylist(event,\''+t+'\')">'+
      '<span class="dim">Añadir</span> <select id="plSel_'+t+'" class="grow">'+opcionesArchivo('')+'</select> '+
      '<button class="btn sm" onclick="addDesdeSelect(\''+t+'\')">＋ Cartel</button>'+
      '<button class="btn sm" onclick="pedirArchivoPlaylist(\''+t+'\')">⬆ Subir</button>'+
      '<button class="btn sm" title="Un hueco que la pantalla rellena sola (el más próximo, por prioridad o sorpresa)" onclick="slotAddAuto(\''+t+'\')">🎭 Hueco automático</button></div>';
    cuerpo='<p class="dim" style="font-size:.78rem;margin:6px 0 10px">En bucle, en orden. Mezcla carteles fijos y huecos automáticos; pon su duración (vídeos: vacío = entero).</p>'+
      '<div class="pllist">'+head+filas+'</div>'+add+
...
function slotItemDup(t,i){ var L=dObj(t).lista, o=L[i]; L.splice(i+1,0, o.auto?{auto:true,segundos:o.segundos,girar:(o.girar!==false)}:{archivo:o.archivo,segundos:o.segundos,girar:(o.girar!==false)}); cambiado(); renderPorDefecto(); preview(); }
function slotAddAuto(t){ var d=dObj(t); if(!d.lista) d.lista=[]; d.lista.push({auto:true,segundos:10,girar:true}); cambiado(); renderPorDefecto(); preview(); }
function slotItemCriterio(t,i,v){ var it=dObj(t).lista[i]; if(v==='proximo') delete it.criterio; else it.criterio=v; cambiado(); renderPorDefecto(); preview(); }
...
// ════════════════════ Guardar ════════════════════
function guardar(){ if(soloLectura())return; api('guardar',{method:'POST',json:{config:config,rev:REV}}).then(function(r){ if(r.rev) REV=r.rev; marcar('Guardado ✓'); toast('Guardado ✓'); }).catch(function(e){ toast(e.msg||'Error al guardar',true); }); }
...
function preview(){
  ...
  var its=[];
  if(s.lista&&s.lista.length){ var autosP=resuelveAutosDe(s.lista); s.lista.forEach(function(it,i){
      if(it.auto){ var ra=autosP[i]; if(ra&&ra.archivo) its.push({archivo:ra.archivo,segundos:(+it.segundos||0),girar:(it.girar!==false)}); return; }
      if(it.archivo) its.push({archivo:it.archivo,segundos:(+it.segundos||0),girar:(it.girar!==false)}); }); }
